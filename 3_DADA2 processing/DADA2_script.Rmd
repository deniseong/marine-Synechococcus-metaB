---
title: "Script DADA2 for petB sequence analysis"
author: "Denise Ong"
date: "8/28/2021"
...
source(here("3_DADA2 processing", "param_dada2_datasetD2.R")) # Change this to the parameter source code
...
## Define directories
```{r} 
source(here("3_DADA2 processing", "directories_DADA2_datasetD2.R"))
```
...
Adapted from https://astrobiomike.github.io/amplicon/dada2_workflow_ex#removing-primers
```{r}
#Step 4: Run cutadapt - This does not work under R studio....

cutadapt <- "/opt/miniconda3/envs/cutadaptenv/bin/cutadapt"
system2(cutadapt, args = "--version") #check if cutadapt has loaded
...
## Dereplicating, sample inference and merging
justConcatenate function=TRUE for merging reads without overlap, 10 N bases added between R1 and R2. N will be ignored for assignTaxonomy (kmer) but not assignSpecies (exact matching). https://github.com/benjjneb/dada2/issues/108
...
```{r}
#remove chimeras
seqtab.nochim <- removeBimeraDenovo(seqtab, method="pooled", multithread=multithread, verbose=TRUE) 
...
```{r}
assigned_asv <- seqtab.nochim_trans %>% 
...
assigned_asv$sequence_hash = purrr::map_chr(assigned_asv$sequence,digest::sha1)
    
write_tsv(assigned_asv, output_path("_Farrant_asv.txt"), na="")
    
assigned_asv_abundance <- seqtab.nochim_trans %>% 
      select(-asv_number, -sequence, -(Domain:Subclade)) %>% 
      gather("file_code", "n_reads", -contains("asv_code")) %>% 
      filter(n_reads > 0 )
    
write_tsv(assigned_asv_abundance, output_path("_Farrant_asv_abundance.txt"), na="")
...
```{r}
seq_out <- Biostrings::DNAStringSet(seqtab.nochim_trans$sequence)
names(seq_out) <- str_c(seqtab.nochim_trans$asv_code,seqtab.nochim_trans$Subclade, sep="|") 
...
# use semi_join to only include samples in the list.
...
saveRDS(ps, output_path("_phyloseq.RDS"))
